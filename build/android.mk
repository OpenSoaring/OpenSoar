...
# A fat-binary build reaches classes.jar / classes.dex / JNI headers
# from each of its four per-ABI submakes.  make serialises a target
# only within one instance, so under -j all four would run javac and
# D8 over the same files at the same time.  The parent builds these
# once and passes NO_ARCH_READY=y, which leaves the submake with no
# recipe for them.  Backport of f79c6067df (#3186).
...
# build libxcsoar.so
# NO_ARCH_READY=y: the noarch outputs are prerequisites above, so they
# are already built; without it all four submakes would rebuild them
# at once.
