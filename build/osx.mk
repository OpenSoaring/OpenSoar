...
APP_NAME = $(PROGRAM_NAME).app
DMG_NAME = $(PROGRAM_NAME).dmg
PKG_NAME = $(PROGRAM_NAME).pkg
MACOS_APP_BUNDLE_IDENTIFIER ?= org.xcsoar.$(PROGRAM_NAME)
...
$(APP_BUNDLE): $(TARGET_BIN_DIR)/$(PROGRAM_NAME) Data/OSX/Info.plist.in.xml $(OSX_LOGO) $(ANGLE_FIX_STAMP)
	...
	$(Q)cp $(TARGET_BIN_DIR)/$(PROGRAM_NAME) $(APP_MACOS)/
	@$(NQ)echo "  COPY    icon"
...
ifeq ($(USE_ANGLE),y)
	...
	$(Q)if ! install_name_tool -change "./libEGL.dylib" "@rpath/libEGL.dylib" $(APP_MACOS)/$(PROGRAM_NAME) 2>/dev/null; then \
		echo "  WARN    Failed to update $(PROGRAM_NAME) libEGL.dylib reference"; \
	fi
	$(Q)if ! install_name_tool -change "./libGLESv2.dylib" "@rpath/libGLESv2.dylib" $(APP_MACOS)/$(PROGRAM_NAME) 2>/dev/null; then \
		echo "  WARN    Failed to update $(PROGRAM_NAME) libGLESv2.dylib reference"; \
	fi
endif
# Bundle Homebrew libraries and fix all @rpath references
	@$(NQ)echo "  BUNDLE  Homebrew libraries"
	$(Q)if ! install_name_tool -add_rpath "@executable_path/../Frameworks" $(APP_MACOS)/$(PROGRAM_NAME) 2>/dev/null; then \
		echo "  WARN    Failed to add rpath to $(PROGRAM_NAME)"; \
	fi
	$(Q)ANGLE_PATTERN="$(if $(ANGLE_PREFIX),|$(ANGLE_PREFIX),)"; \
	for lib in $$(otool -L $(APP_MACOS)/$(PROGRAM_NAME) | \
		...
		if ! install_name_tool -change "$$lib" "@rpath/$$base" $(APP_MACOS)/$(PROGRAM_NAME); then \
			echo "  WARN    Failed to update $(PROGRAM_NAME) reference for $$base"; \
		fi; \
	done
