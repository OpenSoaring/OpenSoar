# This Makefile fragment builds the Android Bundle package (OpenSoar.apk).
...
$(GEN_DIR)/de/$(PROGRAM_NAME_LC)/R.java: $(PROTOBUF_OUT_DIR)/dirstamp
...
# Example: $(eval $(call generate-abi,$(PROGRAM_NAME_LC),armeabi-v7a,ANDROID7))
define generate-abi

ANDROID_LIB_BUILD += $$(ANDROID_BUNDLE_BASE)/lib/$(2)/lib$(1).so

# copy lib$(PROGRAM_NAME_CC).so to ANDROIDFAT
...
# Example: $(eval $(call generate-abi,opensoar))
...
# Debug targets
.DELETE_ON_ERROR: $(ANDROID_BIN)/$(PROGRAM_NAME_CC)-debug.aab
$(ANDROID_BIN)/$(PROGRAM_NAME_CC)-debug.aab: $(BUNDLE_BUILD_DIR)/unsigned.aab $(DEBUG_KEYSTORE) | $(ANDROID_BIN)/dirstamp
	@$(NQ)echo "  SIGN    $@"
	$(Q)cp $< $@
	$(Q)$(JARSIGNER) -keystore $(DEBUG_KEYSTORE) -storepass $(DEBUG_KEY_PASSWORD) $@ $(DEBUG_KEY_ALIAS)

$(ANDROID_BIN)/$(PROGRAM_NAME_CC)-debug.apk: $(ANDROID_BIN)/$(PROGRAM_NAME_CC)-debug.aab $(DEBUG_KEYSTORE)
	@$(NQ)echo "  APK     $@"
...
.DELETE_ON_ERROR: $(ANDROID_BIN)/$(PROGRAM_NAME_CC).aab
$(ANDROID_BIN)/$(PROGRAM_NAME_CC).aab: $(BUNDLE_BUILD_DIR)/unsigned.aab | $(ANDROID_BIN)/dirstamp
	@$(NQ)echo "  SIGN    $@"
	$(Q)cp $< $@
	$(Q)$(JARSIGNER) -keystore $(ANDROID_KEYSTORE) $(JARSIGNER_RELEASE_PASSWD) $@ $(ANDROID_KEY_ALIAS)

$(ANDROID_BIN)/$(PROGRAM_NAME_CC).apk: $(ANDROID_BIN)/$(PROGRAM_NAME_CC).aab
	@$(NQ)echo "  APK     $@"
	$(Q)$(BUNDLETOOL) build-apks --overwrite --mode=universal \
