...
try {  // Configure PEV start window
  ...
  const FineTime  time_f{ time_s };

/* New PEV: ==================================================== */
const auto &open = CommonInterface::Calculated().common_stats.
    start_open_time_span;

  if (open.HasEnded(time_f) || !open.HasBegun(time_f)) {
...
InputEvents::eventExit([[maybe_unused]] const char *misc)
{
  bool force = false;
    ...
    } else if (StringIsEqual(misc, "force")) {
      UI::TopWindow::SetExitValue(EXIT_SYSTEM);
      force = true;
...
void 
InputEvents::eventKeyPressed(const char *misc)
{
//  std::map<unsigned, const char> keys;
    for (auto *p = misc; *p != 0; p++) {
      ...
      ProcessKey(MODE_MENU, ParseKeyCode((const char *)c));
    }
  }
}
#endif
