...
PerformDelete(FileMultiSelectWidget &file_list)
{
  ...
  if (selected.empty()) {
    ShowMessageBox(_("Select at least one file."), C_("Button", "Delete"), MB_OK | MB_ICONINFORMATION);
    return;
  }
  ...
  int rr = ShowMessageBox(prompt, C_("Button", "Delete"), MB_YESNO);
  if (rr != IDYES)
    return;
  ...
  if (failed > 0) {
    ...
    for (const auto &n : failed_names)
      msg.AppendFormat("\n- %s", n.c_str());
    ShowMessageBox(msg, C_("Button", "Delete"), MB_OK | MB_ICONERROR);
  } else {
    StaticString<128> msg;
    msg.Format(_("Deleted %u item(s)."), deleted);
    ShowMessageBox(msg, C_("Button", "Delete"), MB_OK | MB_ICONINFORMATION);
  }
...
ShowAdvancedFileExplorerDialog()
{
  ...
  dialog.AddButton(C_("Button", "Delete"), [&file_list]() { PerformDelete(file_list); });
  dialog.AddButton(_("Rename"), [&file_list]() { PerformRename(file_list); });
  dialog.AddButton(C_("Button", "Select all"), [&file_list]() { file_list.SelectAllFiles(); });
