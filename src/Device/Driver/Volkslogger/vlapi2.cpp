...
VLAPI_DATA::DECLARATION::get(const DBB &dbb)
{
  ...
  if ((p = dbb.fdf_findfield(FLDPLT1))>=0)
    CopyString(plt1, sizeof(plt1), (const char *)dbb.GetFDF(p + 2));
  if ((p = dbb.fdf_findfield(FLDPLT2))>=0)
    CopyString(plt2, sizeof(plt2), (const char *)dbb.GetFDF(p + 2));
  if ((p = dbb.fdf_findfield(FLDPLT3))>=0)
    CopyString(plt3, sizeof(plt3), (const char *)dbb.GetFDF(p + 2));
  if ((p = dbb.fdf_findfield(FLDPLT4))>=0)
    CopyString(plt4, sizeof(plt4), (const char *)dbb.GetFDF(p + 2));
  ...
  if ((p = dbb.fdf_findfield(FLDGTY))>=0)
    CopyString(flightinfo.glidertype, sizeof(flightinfo.glidertype),
               (const char *)dbb.GetFDF(p + 2));
  if ((p = dbb.fdf_findfield(FLDGID))>=0)
    CopyString(flightinfo.gliderid, sizeof(flightinfo.gliderid),
               (const char *)dbb.GetFDF(p + 2));
  if ((p = dbb.fdf_findfield(FLDCCL))>=0)
    CopyString(flightinfo.competitionclass,
               sizeof(flightinfo.competitionclass),
               (const char *)dbb.GetFDF(p + 2));
  if ((p = dbb.fdf_findfield(FLDCID))>=0)
    CopyString(flightinfo.competitionid, sizeof(flightinfo.competitionid),
               (const char *)dbb.GetFDF(p + 2));
