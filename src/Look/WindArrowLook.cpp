...
WindArrowLook::Initialise(const Font &_font, bool inverse)
{
  ...
  const Color color_external(0x80, 0x80, 0xff);  // a light blue
  arrow_brush_external.Create(IsDithered()
                              ? color_external
                              : ColorWithAlpha(color_external, ALPHA_OVERLAY));

  const Color color_instantaneous(0x80, 0xff, 0x80);  // a light green
  arrow_brush_instantaneous.Create(IsDithered()
                                   ? color_instantaneous
                                   : ColorWithAlpha(color_instantaneous,
                                                    ALPHA_OVERLAY));
