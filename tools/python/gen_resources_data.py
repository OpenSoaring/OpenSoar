...
# the icon densities, named and zoomed the way upstream's
# build/resource.mk renders them (ldpi = 1.0 at 96dpi)
ICON_ZOOM = {"ldpi": 1.0, "mdpi": 1.6667, "xhdpi": 3.3333, "xxhdpi": 5.0}
...
def pretty_path(path: str) -> str:
    """a readable name for the comment above each blob

    relpath() raises on Windows when the file and the working directory
    sit on different drives - source on C:, build directory on E:, say -
    and a comment is not worth failing the build for.
    """
    try:
...
def main() -> int:
    ...
    for line in preprocess(resources, defines):
        ...
        if kind == "bitmap_icon_scaled":
            blobs.append((f"resource_{ident}", make_icon(t, name, "ldpi")))
            blobs.append((f"resource_{ident}_MDPI", make_icon(t, name, "mdpi")))
            blobs.append((f"resource_{ident}_XHDPI", make_icon(t, name, "xhdpi")))
            blobs.append((f"resource_{ident}_XXHDPI", make_icon(t, name, "xxhdpi")))
